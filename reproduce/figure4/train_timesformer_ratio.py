"""
train_timesformer_ratio.py — TimeSformer (ViT-Base, Kinetics-400 Pretrained) Training
3-Fold Cross-Validation with Video-Level Data Efficiency Analysis

Usage:
    python train_timesformer_ratio.py                                         # Default: 100% data
    python train_timesformer_ratio.py --train_data_ratio 0.75                 # 75% of training videos
    python train_timesformer_ratio.py --train_data_ratio 0.5 --video_split_seed 123
    python train_timesformer_ratio.py --test_folds 2 --train_folds 1 3
"""
...
DEFAULTS = dict(
    ...
    seed=2025,
    model_save_dir="checkpoints/timesformer_ratio",
...
def parse_args():
    p = argparse.ArgumentParser(description="Train TimeSformer with data efficiency analysis")
...
def main():
    ...
    fold_tag = (f"timesformer_train_{'_'.join(map(str, args.train_folds))}"
                f"_val_{'_'.join(map(str, args.test_folds))}_{ratio_tag}{split_tag}")

    print(f"\n{'='*70}")
    print(f"TimeSformer (ViT-Base, K400) Training (Data Efficiency) — {fold_tag}")
    print(f"{'='*70}")
    ...
    train_frame_counts, total_train_frames = count_frames_per_behavior(
        train_vids_used, train_labs_used, o2n,
