"""
eval_swin3d_ratio.py — Evaluate a trained Video Swin-T model (ratio variant)
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate Video Swin-T model (ratio variant)")
    ...
    p.add_argument("--save_cm", type=str, default=None, help="Path to save confusion matrix image")
    return p.parse_args()
...
class WindowPredictionDataset(Dataset):
    ...
    def _generate_windows(self):
        ...
        for vp, lp in zip(self.video_paths, self.label_paths):
            ...
            if len(labels_oh) != T:
                continue
            remap, valid = filter_and_remap_labels(labels_oh, self.o2n)
...
def build_swin3d(num_classes, hidden_dim=512, dropout=0.3):
...
def main():
    ...
    print(f"\n{'='*80}")
    print("MODEL EVALUATION — Video Swin-T (ratio variant)")
    print(f"{'='*80}")
    ...
    if args.save_cm:
        plot_confusion_matrix(metrics["cm"], SELECTED_BEHAVIORS, args.save_cm,
                              f"Swin3D Eval (F1={metrics['f1_macro']:.4f})")
