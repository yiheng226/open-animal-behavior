"""
train_swin3d_ratio.py — Video Swin Transformer-T (Kinetics-400 Pretrained) Training
3-Fold Cross-Validation with Video-Level Data Efficiency Analysis

Usage:
    python train_swin3d_ratio.py                                        # Default: 100% data
    python train_swin3d_ratio.py --train_data_ratio 0.75                # 75% of training videos
    python train_swin3d_ratio.py --train_data_ratio 0.5 --video_split_seed 123
    python train_swin3d_ratio.py --test_folds 2 --train_folds 1 3
"""
...
DEFAULTS = dict(
    ...
    seed=2025,
    model_save_dir="checkpoints/swin3d_ratio",
...
def parse_args():
    p = argparse.ArgumentParser(description="Train Video Swin-T with data efficiency analysis")
    ...
    p.add_argument("--train_data_ratio", type=float, default=DEFAULTS["train_data_ratio"],
                   help="Fraction of training videos to use (1.0=all, 0.5=half)")
    p.add_argument("--video_split_seed", type=int, default=DEFAULTS["video_split_seed"],
                   help="RNG seed for video-level subsampling (vary for repeated experiments)")
    return p.parse_args()
...
def subsample_videos_by_ratio(video_paths, label_paths, ratio, rng_seed=42):
    """Video-level subsampling for Data Efficiency Analysis.

    Subsample at the video level, not the window level.
    Because sliding window stride is small, adjacent windows overlap heavily.
    If subsampling at window level, even 50% means the model sees almost all frames.

    Correct approach: selected videos produce all their windows normally;
    unselected videos are completely excluded.

    Reference: DeepEthogram (eLife 2021) uses video-level data efficiency experiments,
    running multiple seeds per ratio and averaging.
    """
    if ratio >= 1.0:
        return video_paths, label_paths

    n_total = len(video_paths)
    n_select = max(1, int(round(n_total * ratio)))

    rng = np.random.RandomState(rng_seed)
    selected_indices = sorted(rng.choice(n_total, size=n_select, replace=False).tolist())

    selected_videos = [video_paths[i] for i in selected_indices]
    selected_labels = [label_paths[i] for i in selected_indices]

    return selected_videos, selected_labels


def count_frames_per_behavior(video_paths, label_paths, o2n, num_behaviors, num_classes):
    """Count per-behavior frame totals across selected videos."""
    frame_counts = {c: 0 for c in range(num_classes)}
    total_valid = 0
    for vp, lp in zip(video_paths, label_paths):
        df = pd.read_csv(lp)
        oh = df.iloc[:, 0:num_behaviors].values
        remapped, valid = filter_and_remap_labels(oh, o2n)
        for i in range(len(remapped)):
            if valid[i]:
                frame_counts[remapped[i]] += 1
                total_valid += 1
    return frame_counts, total_valid
...
def main():
    ...
    fold_tag = (f"swin3d_train_{'_'.join(map(str, args.train_folds))}"
                f"_val_{'_'.join(map(str, args.test_folds))}_{ratio_tag}{split_tag}")

    print(f"\n{'='*70}")
    print(f"Video Swin-T Training (Data Efficiency) — {fold_tag}")
    print(f"{'='*70}")
    ...
    if args.train_data_ratio < 1.0:
        print(f"  Video split seed: {args.video_split_seed}")
    print(f"{'='*70}\n")
    ...
    print(f"Total train: {len(train_vids)} videos | Val: {len(val_vids)} videos")

    # Video-level subsampling
    train_vids_used, train_labs_used = subsample_videos_by_ratio(
        train_vids, train_labs, args.train_data_ratio, args.video_split_seed)
    print(f"Training videos used: {len(train_vids_used)} / {len(train_vids)}")
    if args.train_data_ratio < 1.0:
        print(f"  Selected: {[os.path.basename(v) for v in train_vids_used]}")
    print()
    ...
    train_ds = SlidingWindowVideoDataset(
        train_vids_used, train_labs_used, args.window_size, args.stride,
    ...
    train_frame_counts, total_train_frames = count_frames_per_behavior(
        train_vids_used, train_labs_used, original_to_new,
        len(ALL_BEHAVIOR_NAMES), num_classes)

    # Data Efficiency Report
    print(f"{'='*65}")
    print(f"📊 Data Efficiency Report  (ratio={args.train_data_ratio}, "
          f"video_split_seed={args.video_split_seed})")
    print(f"{'='*65}")
    print(f"  Training videos: {len(train_vids_used)} / {len(train_vids)}")
    print(f"  Total valid training frames: {total_train_frames}")
    print(f"  Total training windows:      {len(train_ds)}")
    print(f"  Val windows:                 {len(val_ds)}")
    total_windows = sum(counts.values())
    print(f"\n  {'Behavior':>15}  {'Frames':>8}  {'Windows':>8}  {'Frame%':>7}  {'Win%':>7}")
    print(f"  {'-'*53}")
    for c in range(num_classes):
        fc = train_frame_counts.get(c, 0)
        wc = counts.get(c, 0)
        fpct = 100 * fc / total_train_frames if total_train_frames > 0 else 0
        wpct = 100 * wc / total_windows if total_windows > 0 else 0
        print(f"  {selected[c]:>15}  {fc:>8}  {wc:>8}  {fpct:>6.2f}%  {wpct:>6.2f}%")
    print(f"{'='*65}\n")
    ...
    for epoch in range(args.num_epochs):
        ...
        log.append({
            ...
            "video_split_seed": args.video_split_seed,
            "train_videos": len(train_vids_used),
            "train_videos_selected": [os.path.basename(v) for v in train_vids_used],
            "train_windows_used": len(train_ds),
            "train_frames_per_behavior": {selected[c]: train_frame_counts[c] for c in range(num_classes)},
            "train_windows_per_behavior": {selected[c]: counts.get(c, 0) for c in range(num_classes)},
        })
        print()
    ...
    with open(log_path, "w") as f:
        json.dump({
            ...
            "config": {**vars(args), "model": "Video Swin-T (K400 Pretrained)"},
        }, f, indent=2)
